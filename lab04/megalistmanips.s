...
mapLoop:
	...
	slli t2, t2, 2							 # 1: changing size to byte offset

    add t1, t1, t0      					 # offset the array address by the count
    lw a0, 0(t1)        					 # load the value at that address into a0

	# 2: saving temp registers    
    addi sp, sp, -12
    sw t0, 0(sp)
    sw t1, 4(sp)
    sw t2, 8(sp)
    ...
    addi t0, t0, 4                           # 4: increment the count
    bne t0, t2, mapLoop                      # repeat if we haven't reached the array size yet
    
    lw a0, 8(s0)                             # 5: load the address of the next node into a0
    
    mv a1, s1                                # 6: put the address of the function back into a1 to prepare for the recursion
